	...
	// Main textarea
	{
				...
				OfdxAsync.send({
					...
					ondone: function(http){
						...
						if(http.status === 200){
							...
							note_texted_field.value = OfdxAsync.atob(http.response.body);
							note_texted_title.value = OfdxAsync.atob(http.response.title);
						} else {
							// FIXME debug
		...
		var changeFunction = function(e){
			...
			changeTimeout = setTimeout(function(){
				...
				if(notename){
					let putbody = {
						body: OfdxAsync.btoa(note_texted_field.value),
						title: OfdxAsync.btoa(note_texted_title.value)
					};
				...
				ondone: function(http){
					if(http.status === 200){
						OfdxSomeNotes.notes = http.response.notes
							// Decode base64 title.
							.map(el => {
								return { ... el, title: OfdxAsync.atob(el.title) };
							})
